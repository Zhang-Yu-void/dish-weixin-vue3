...
/** 树权限（展开/折叠）*/
function handleCheckedTreeExpand(value: import('element-plus').CheckboxValueType, type: string) {
  const checked = value === true
  if (type == 'menu') {
    let treeList = menuOptions.value
    for (let i = 0; i < treeList.length; i++) {
      const nodeId = treeList[i].id
      if (nodeId !== undefined) menuRef.value.store.nodesMap[nodeId].expanded = checked
    }
  } else if (type == 'dept') {
    let treeList = deptOptions.value
    for (let i = 0; i < treeList.length; i++) {
      const nodeId = treeList[i].id
      if (nodeId !== undefined) deptRef.value.store.nodesMap[nodeId].expanded = checked
    }
  }
}

/** 树权限（全选/全不选） */
function handleCheckedTreeNodeAll(value: import('element-plus').CheckboxValueType, type: string) {
  const checked = value === true
  if (type == 'menu') {
...
/** 树权限（父子联动） */
function handleCheckedTreeConnect(value: import('element-plus').CheckboxValueType, type: string) {
  const checked = value === true
  if (type == 'menu') {
