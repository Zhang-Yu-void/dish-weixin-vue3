...
function getTreeselect() {
  ...
  listMenu().then((response) => {
    const menu: { menuId: number; menuName: string; children: any[] } = { menuId: 0, menuName: '主类目', children: [] }
...
function recordOriginalOrders(list: SysMenu[]) {
  list.forEach((item) => {
    if (item.menuId !== undefined) originalOrders.value[item.menuId] = item.orderNum ?? 0
